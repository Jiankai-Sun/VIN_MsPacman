...
def dqn_learing(
    ...
    target_Q = q_func(config).type(dtype)
    ...
    for t in count():
        ...
        if (t > learning_starts and
            ...
            next_max_q = target_Q(next_obs_batch, config).detach().max(1)[0]
        ...
        if t % LOG_EVERY_N_STEPS == 0 and t > learning_starts:
            ...
            with open('model/statistics.pkl', 'wb') as f:
                pickle.dump(Statistic, f)
                print("Saved to %s" % 'model/statistics.pkl')

            torch.save(Q.state_dict(), "model/vin_with_moving_obstacles_{}.pth".format(t))